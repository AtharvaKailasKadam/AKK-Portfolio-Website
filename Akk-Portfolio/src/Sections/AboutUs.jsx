        ...
        <>
            <div className='w-full'>
                <section id="about" className="min-h-screen flex items-center justify-center relative bg-black text-white overflow-hidden">
                    ...
                    <div className="relative z-10 max-w-6xl w-full mx-auto px-4 sm:px-6 lg:px-12 py-12 sm:py-20 flex flex-col gap-8 sm:gap-12">
                        <motion.div className="flex flex-col md:flex-row items-center md:items-stretch gap-6 sm:gap-8"
                        initial={{ opacity: 0, y: 24 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.6 }}
                        //Excutes the Animation only Once.
                        viewport={{ once: true, amount: 0.5 }}>
                            <motion.div className="relative w-[160px] h-[180px] sm:w-[200px] sm:h-[220px] md:w-[220px] md:h-[240px] rounded-2xl hover:scale-105 shadow-2xl bg-gradient-to-r from-[#1cd8d2]/20 to-[302b63]/20 border border-[white] overflow-hidden flex-shrink-0">
                                <img src={aboutImg} alt="MySelf" className="absolute inset-0" />
                            </motion.div>

                            <div className="flex-1 flex flex-col justify-center text-center md:text-left">
                                <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent
                                bg-gradient-to-r from-[#1cd8d2] via-[#00bf8f] to-[#1cd8d2]">
                                    Atharva Kailas Kadam
                                </h2>
                                <p className="mt-2 text-base sm:text-lg md:text-xl text-white/99 font-semibold">
                                    Web Developer
                                </p>
                                ...
                                <div className="mt-6 grid grid-cols-2 gap-3 sm:gap-4 max-w-xl mx-auto md:mx-0">
                                    {stats.map((stat, index) => (
                                        <motion.div key={index} className="rounded-lg sm:rounded-xl border border-[white]/10 bg-[white]/5 p-2 sm:p-4"
                                        initial={{ opacity: 0, y: 20 }}
                                        whileInView={{ opacity: 1, y: 0 }}
                                        transition={{ duration: 0.4 }}
                                        viewport={{ once: true, amount: 0.5 }}>
                                            <p className="text-xs sm:text-sm text-gray-300 font-medium"><u><b>{stat.label}</b></u></p>
                                            <p className="text-sm sm:text-base font-semibold text-white/99">{stat.value}</p>
                                        </motion.div>
                                    ))}
                                </div>
